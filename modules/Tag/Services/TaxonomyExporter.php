...
final class TaxonomyExporter
{
    public function export(): TaxonomyDocument
    {
        return new TaxonomyDocument(
            categories: TagCategory::query()->orderBy('name')->get()
                ->map(static fn (TagCategory $category): array => [
                    'name' => $category->name,
                    'color' => $category->color,
                    'sort_order' => $category->sort_order,
                ])->all(),
            tags: Tag::query()->with('category')->orderBy('name')->get()
                ->map(static fn (Tag $tag): array => [
                    'name' => $tag->name,
                    'category' => $tag->category?->name,
                    'description' => $tag->description,
                ])->all(),
            aliases: TagAlias::query()->with('tag')->orderBy('alias_name')->get()
                ->map(static fn (TagAlias $alias): array => [
                    'alias' => $alias->alias_name,
                    'tag' => $alias->tag->name,
                ])->all(),
            implications: TagImplication::query()->with(['tag', 'impliedTag'])->get()
