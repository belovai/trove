...
<template>
    ...
    <div class="flex flex-col gap-4">
        ...
        <p v-if="props.tags.data.length === 0" class="text-sm text-gray-500">{{ t('tag::tag.no_results') }}</p>

        <div v-else class="flex flex-wrap gap-2">
            <TagChip v-for="tag in props.tags.data" :key="tag.name" :tag="tag" />
        </div>

        <nav v-if="props.tags.last_page > 1" class="flex items-center justify-center gap-4 text-sm">
            <Link v-if="props.tags.prev_page_url" :href="props.tags.prev_page_url">&larr;</Link>
            <span>{{ props.tags.current_page }} / {{ props.tags.last_page }}</span>
            <Link v-if="props.tags.next_page_url" :href="props.tags.next_page_url">&rarr;</Link>
        </nav>
    </div>
</template>
